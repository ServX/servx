...
#if defined(NDEBUG)
# error "Servx cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5b530258a646546d2a5318d294ee028f526ae83f112b941c977750d9de6a0299");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Servx: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Servx Signed Message:\n";
...
{
    // Servx: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60; // Servx: 1 block
static const int64 nTargetSpacing = 60; // Servx: 1 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int switchToNew = 4500;
static const int64 nTargetTimespanNew = 45;
static const int64 nTargetSpacingNew = 45;
static const int64 nIntervalNew = nTargetTimespanNew / nTargetSpacingNew;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Servx: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    if(pindexLast->nHeight > switchToNew){
        //45 seconds blocktime
        int64 nActualTimespanMax = nTargetTimespanNew * (112/100); //12% down
        int64 nActualTimespanMin = nTargetTimespanNew * (100/111); //10% up
...
unsigned int static GetNextWorkRequired_DigiShield(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64 retargetTimespan = 45;
    int64 retargetSpacing = 45;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Servx: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Servx: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ServX We Trust";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x2d4398c243e6c644fdf10c51fe301f1e64321efb4f03b415fd40396b1d9e11c0"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Servx: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ServxMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ServxMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ServxMiner(CWallet *pwallet)
{
    printf("ServxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("servx-miner");
    ...
    try { loop {
        ...
        printf("Running ServxMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ServxMiner, pwallet));
}
